...
async def load_trip(db: AsyncSession, session_id: str) -> tuple[dict, str]:
...
async def get_itinerary(db: AsyncSession, session_id: str) -> dict | None:
    result = await db.execute(select(TripSession).where(TripSession.session_id == session_id))
    row = result.scalar_one_or_none()
    return row.itinerary if row else None
